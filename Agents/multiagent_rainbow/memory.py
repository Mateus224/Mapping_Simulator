...
map_size=32
Transition_dtype = np.dtype([('timestep', np.int32), ('state', np.float32, (5, map_size, map_size)), ('action', np.int32), ('reward', np.float32), ('nonterminal', np.bool_)])
blank_trans = (0, np.zeros((5, map_size, map_size), dtype=np.float32), 0, 0.0, False)
...
class ReplayMemory():
  def __init__(self, args, capacity):
    ...
    self.history = 1#args.history_length
    self.discount = args.discount
    self.n = 1 #args.multi_step
    self.priority_weight = args.priority_weight  # Initial importance sampling weight β, annealed to 1 over course of training
    self.priority_exponent = 0.6#args.priority_exponent
  ...
  def _get_samples_from_segments(self, batch_size, p_total):
    ...
    all_states = np.copy(transitions['state'])
    #print(all_states.shape)
    all_states = np.squeeze(all_states)
    states = torch.tensor(all_states[:,0], device=self.device, dtype=torch.float32)
    next_states = torch.tensor(all_states[:,1], device=self.device, dtype=torch.float32)
